...
function ConfirmDeleteModal({
  isOpen, taskTitle, isLoading, onConfirm, onCancel,
}: {
  isOpen: boolean
  taskTitle: string
  isLoading: boolean
  ...
  return (
    <div
      ref={backdropRef}
      onClick={(e) => { if (e.target === backdropRef.current && !isLoading) onCancel() }}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
      role="dialog"
      aria-modal="true"
    >
      <div className="relative w-full max-w-sm bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl shadow-black/50 p-6">
        {/* Warning icon */}
        ...
        <h3 className="text-base font-semibold text-gray-100 text-center">Delete task?</h3>
        <p className="mt-2 text-sm text-gray-400 text-center leading-relaxed">
          Are you sure you want to delete <span className="text-gray-200 font-medium">&quot;{taskTitle}&quot;</span>? This action cannot be undone.
        </p>
        ...
        <div className="mt-6 flex items-center gap-3">
          <button
            onClick={onCancel}
            disabled={isLoading}
            className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:text-gray-200 hover:bg-gray-800 border border-gray-700 transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold bg-red-500 hover:bg-red-400 text-white shadow-lg shadow-red-500/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isLoading && (
...
export default function DashboardPage() {
  ...
  }, [filter, search, page, fetchTasks, isAuthenticated])
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sorted.map(task => (
              <TaskCard key={task.id} task={task} onEdit={openEdit} onDelete={requestDelete} onToggle={handleToggle} />
            ))}
          </div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-gray-800">
            ...
            {sorted.map(task => (
              <TaskRow key={task.id} task={task} onEdit={openEdit} onDelete={requestDelete} onToggle={handleToggle} />
            ))}
          </div>
        )}
