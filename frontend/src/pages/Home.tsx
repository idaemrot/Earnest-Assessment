    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 overflow-hidden relative selection:bg-teal-500/30">
      ...
      <main className="relative z-10 w-full max-w-6xl mx-auto px-6 pt-24 pb-32 sm:pt-32 sm:pb-40 flex flex-col items-center text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 animate-fade-in-up animation-delay-500">
          ...
          <button
            onClick={() => isAuthenticated ? null : openAuth('login')}
            className={`px-8 py-3.5 bg-gray-900 border border-gray-800 hover:border-gray-700 hover:bg-gray-800 rounded-full font-medium text-white transition-all w-full sm:w-auto group flex items-center justify-center gap-2 ${isAuthenticated ? 'opacity-0 pointer-events-none' : 'active:scale-95'}`}
          >
            <span>Live Demo</span>
            <svg className="h-4 w-4 text-gray-500 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      {/* ─── Animations via standard tailwind arbitrary values or global CSS  ─ */}
      {/* We inline standard CSS to avoid editing tailwind.config.ts if it isn't strictly necessary, 
          though standard tailwind classes usually suffice. We will use a style block for custom animations. */}
      <style>{`
        @keyframes fadeInUp {
