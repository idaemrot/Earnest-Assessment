    ...
    <div className="mt-2 space-y-1.5">
      ...
      <ul className="grid grid-cols-3 gap-x-2">
        {[{ label: '8+ chars', ok: hasLength(password) }, { label: 'Uppercase', ok: hasUpper(password) }, { label: 'Number', ok: hasNumber(password) }].map(({ label, ok }) => (
          <li key={label} className={`flex items-center gap-1 text-xs ${ok ? 'text-teal-400' : 'text-gray-600'}`}>
...
export default function Register() {
  ...
  const validate = (): boolean => {
    ...
    else if (!hasLength(password))                         errs.password = 'Must be at least 8 characters'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAxiosError(err)) {
        ...
        // Handle Zod field errors or plain message
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="reg-password" type={showPw ? 'text' : 'password'} autoComplete="new-password" value={password}
              onChange={e => { setPassword(e.target.value); clearError('password') }}
              placeholder="••••••••" className={inputCls(errors.password)}
            />
            <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-400 transition-colors" aria-label="Toggle password">{showEyeIcon(showPw)}</button>
          </div>
          {errors.password ? <p className="mt-1.5 flex items-center gap-1 text-xs text-red-400">{errIcon}{errors.password}</p> : <StrengthBar password={password} />}
        </div>

        {/* Confirm */}
        <div>
          <label htmlFor="reg-confirm" className="block text-xs font-medium text-gray-400 mb-1.5">Confirm password</label>
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowConfirm(v => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-400 transition-colors" aria-label="Toggle confirm">{showEyeIcon(showConfirm)}</button>
          </div>
        ...
        <button type="submit" disabled={loading}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold bg-teal-500 hover:bg-teal-400 text-white shadow-lg shadow-teal-500/25 transition-all disabled:opacity-60 disabled:cursor-not-allowed mt-1"
        >
          {loading ? <><svg className="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Creating account…</> : 'Create account'}
        </button>
      </form>
    </AuthLayout>
  )
